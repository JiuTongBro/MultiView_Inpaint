...
class GeneralConditioner(nn.Module):
    ...
    def get_unconditional_conditioning(
        ...
        if 'num_video_frames' in batch_c.keys():
            for k in ["crossattn", "concat"]:
                c[k] = repeat(c[k], "b ... -> (b t) ...", t=batch_c['num_video_frames'])

        if additional_cond_keys is not None:
            cond_keys = additional_cond_keys.intersection(batch_c)
            for k in cond_keys:
                if k in ['crossattn_scale', 'concat_scale', 'prev_frame']:
                    c[k] = repeat(batch_c[k], "b ... -> (b t) ...", t=batch_c['num_video_frames'])
                else:
                    c[k] = batch_c[k]

        batch_u = batch_c if batch_uc is None else batch_uc
        uc = self(batch_u, force_uc_zero_embeddings)

        if 'num_video_frames' in batch_u.keys():
            for k in ["crossattn", "concat"]:
                uc[k] = repeat(uc[k], "b ... -> (b t) ...", t=batch_u['num_video_frames'])

        if additional_cond_keys is not None:
            cond_keys = additional_cond_keys.intersection(batch_u)
            for k in cond_keys:
                if k in ['crossattn_scale', 'concat_scale', 'prev_frame']:
                    uc[k] = repeat(batch_u[k], "b ... -> (b t) ...", t=batch_u['num_video_frames'])
                else:
                    uc[k] = batch_u[k]
